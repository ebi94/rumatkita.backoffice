  ...
  const columns = [
    {
      ...
      renderCell: ({ row }) => (
        <LinkStyled
          href={`/order-telenursing/detail/${row?.booking_code}`}>{`#${row?.booking_code}`}</LinkStyled>
      )
    },
    {
      flex: 0.2,
  ...
  return (
    <DatePickerWrapper>
      <Grid container spacing={6}>
        <PageHeader
          title={<TypographyStyled variant="h5">Order Telenursing List</TypographyStyled>}
          subtitle={
            <Typography variant="body2">
              List of Customer Order Telenursing using RUMATKITA Apps
            </Typography>
          }
        />
        <Grid item xs={12}>
